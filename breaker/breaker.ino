...
// ✅ 서버 정보 (Flask 서버)
const char* serverIP = "192.168.0.22";  
...
void processCommand(String receivedMessage) {
  ...
  for (int i = 0; i < receivedMessage.length(); i++) {
    if (i == 0) {
      carCount = receivedMessage[i];
    } else {
      command += receivedMessage[i];
    }
  }

  Serial.println("차량 수: " + carCount);
  ...
  if (command == "PASS") 
  {
    ...
    delay(2000);
    myServo.write(0);
    client.print("in/PASSOK\n");
    oledDisplay("남은 주차장 : " + carCount);
  }
  ...
  else if (command == "CLOSE") 
  {
    ...
    myServo.write(0);
    oledDisplay("차단기 닫힘!");
    client.print("in/CLOSEOK\n");
  }
  else if (command == "COUNT") 
  {
    Serial.println("🚗 차량 주차 수 변경!");
    oledDisplay("남은 주차장 : " + carCount);    
  } 
